...
import { searchPath, upcomingPath } from '../../statics/urls'
...
const ComingSoon = () => {
  ...
  const fetchmovies = async ({pagenumber}) => {
    let upcomings = fetchAllMovie(`${upcomingPath({ page: pagenumber })}`);
    try {
      if ((await upcomings).status === 200) {
        setUpcomings((await upcomings).data.results);
        setTotalPage((await upcomings).data.total_pages)
      } else {
